...
function ConnectionManager:Cleanup()
    ...
    if autoSaveTimer then
        pcall(function() task.cancel(autoSaveTimer) end)
        autoSaveTimer = nil
    end
    ...
    if WebhookModule and WebhookModule.Stop then
        pcall(function() WebhookModule:Stop() end)
    end
    
    local AutoFavorite = GetModule("AutoFavorite")
    if AutoFavorite and AutoFavorite.Stop then
        pcall(function() AutoFavorite:Stop() end)
    end
...
if AutoFavorite then
    ToggleReferences.AutoFavorite = makeToggle(catAutoFav, "Enable Auto Favorite", function(on)
        ConfigSystem.Set("AutoFavorite.Enabled", on)
        if on then
            AutoFavorite:Start()
        else
            AutoFavorite:Stop()
...
makeButton(catConfig, "🔄 Load Config", function()
    if ConfigSystem then
        ...
        if success then
            ...
            SendNotification("Config", "✓ Configuration loaded!", 3)
            -- Re-apply loaded config to UI and modules
            task.delay(0.1, function()
                pcall(ApplyLoadedConfig)
            end)
        else
...
local function ApplyLoadedConfig()
    ...
    local applyThread = task.delay(0.5, function()
        local toggleConfigs = {
...
task.spawn(function()
    while true do
        task.wait(60)  -- Collect garbage every 60 seconds
