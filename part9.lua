-- LynxGUI v2.3.1 Performance Optimized - MANUAL SAVE VERSION
...
-- ✅ FIXED: Declare icon variable at the top
local icon = nil
...
local function makeDropdown(parent, title, icon, items, onSelect, uniqueId, defaultValue)
    ...
    if defaultValue and table.find(items, defaultValue) then
        ...
        ConnectionManager:AddThread(thread)
    end
...
-- ============================================
-- CONFIG SYSTEM (MANUAL SAVE VERSION - NO AUTO SAVE)
-- ============================================
...
-- ✅ REMOVED: Auto-save system, dirty flags, and auto-save timer
-- Now all config changes are manual - user must click "Save Config" button

-- Simple set function without auto-save
...
local savedInstantMode = "Fast"  -- Default value, not loaded from config
local savedFishingDelay = 1.30
local savedCancelDelay = 0.19
local savedInstantEnabled = false
...
-- ✅ REMOVED: Auto-load saved config on startup for instant fishing
...
local savedBlatantTesterCompleteDelay = 0.5
local savedBlatantTesterCancelDelay = 0.1
...
local savedBlatantV1CompleteDelay = 0.05
local savedBlatantV1CancelDelay = 0.1
...
local savedUltraBlatantCompleteDelay = 0.05
local savedUltraBlatantCancelDelay = 0.1
...
local tierSys, varSys

if AutoFavorite then
    tierSys = makeCheckboxDropdown(catAutoFav, "Tier Filter", AutoFavorite.GetAllTiers(), {
    ...
    -- ✅ REMOVED: Auto-load saved tiers and variants
end
...
local selectedEventName = nil  -- Default nil, not loaded from config
...
if AutoSellTimer then
    ...
    ToggleReferences.AutoSellTimer = makeToggle(catTimer, "Auto Sell Timer", function(on)
        ...
        if AutoSellTimer then
            pcall(function()
                if on then
                    local interval = 5  -- Default value
...
local currentWebhookURL = ""  -- Default empty, not loaded from config
local currentDiscordID = ""  -- Default empty, not loaded from config
...
local freecamStatusContainer = new("Frame", {
    Parent = catFreecam,
    Size = UDim2.new(1, 0, 0, 60),
    BackgroundColor3 = colors.bg3,
    BackgroundTransparency = 0.8,
...
new("UICorner", {Parent = freecamStatusContainer, CornerRadius = UDim.new(0, 8)})

local freecamStatusText = new("TextLabel", {
    Parent = freecamStatusContainer,
    Size = UDim2.new(1, -24, 1, -24),
    Position = UDim2.new(0, 12, 0, 12),
    BackgroundTransparency = 1,
    Text = "📹 FREECAM CONTROLS\n\n" .. (isMobile and "Toggle: Use button below" or "Toggle: Press F3\nMove: WASD | Up/Down: Space/Shift"),
    Font = Enum.Font.GothamBold,
    TextSize = 9,
    TextColor3 = colors.text,
    TextWrapped = true,
...
local currentFakeName = "Guest"  -- Default value, not loaded from config
local currentFakeLevel = "1"  -- Default value, not loaded from config
...
-- ✅ MANUAL CONFIG MANAGEMENT SECTION
local catConfig = makeCategory(settingsPage, "Configuration Management", "💾")

-- Helper function to load config and apply to all UI elements
local function LoadConfigToUI()
    ...
    -- Apply all saved values to UI toggles
    local toggleConfigs = {
        ...
        {ref = "Sprint", path = "Settings.Sprint", default = false},
        {ref = "InfiniteJump", path = "Settings.InfiniteJump", default = false},
    }
    ...
    -- Apply instant fishing settings
    pcall(function()
        savedInstantMode = ConfigSystem.Get("InstantFishing.Mode", "Fast")
        savedFishingDelay = ConfigSystem.Get("InstantFishing.FishingDelay", 1.30)
        savedCancelDelay = ConfigSystem.Get("InstantFishing.CancelDelay", 0.19)
        currentInstantMode = savedInstantMode
        fishingDelayValue = savedFishingDelay
        cancelDelayValue = savedCancelDelay
    ...
    pcall(function()
        savedBlatantTesterCompleteDelay = ConfigSystem.Get("BlatantTester.CompleteDelay", 0.5)
        savedBlatantTesterCancelDelay = ConfigSystem.Get("BlatantTester.CancelDelay", 0.1)
        local blatantv2fix = GetModule("blatantv2fix")
        if blatantv2fix then
            blatantv2fix.Settings.CompleteDelay = savedBlatantTesterCompleteDelay
            blatantv2fix.Settings.CancelDelay = savedBlatantTesterCancelDelay
    ...
    pcall(function()
        savedBlatantV1CompleteDelay = ConfigSystem.Get("BlatantV1.CompleteDelay", 0.05)
        savedBlatantV1CancelDelay = ConfigSystem.Get("BlatantV1.CancelDelay", 0.1)
        local blatantv1 = GetModule("blatantv1")
        if blatantv1 then
            blatantv1.Settings.CompleteDelay = savedBlatantV1CompleteDelay
            blatantv1.Settings.CancelDelay = savedBlatantV1CancelDelay
    ...
    pcall(function()
        savedUltraBlatantCompleteDelay = ConfigSystem.Get("UltraBlatant.CompleteDelay", 0.05)
        savedUltraBlatantCancelDelay = ConfigSystem.Get("UltraBlatant.CancelDelay", 0.1)
        local UltraBlatant = GetModule("UltraBlatant")
        if UltraBlatant then
            if UltraBlatant.Settings then
                UltraBlatant.Settings.CompleteDelay = savedUltraBlatantCompleteDelay
    ...
    -- Apply Auto Favorite settings
    pcall(function()
        if tierSys and varSys then
            local savedTiers = ConfigSystem.Get("AutoFavorite.EnabledTiers", {})
            local savedVariants = ConfigSystem.Get("AutoFavorite.EnabledVariants", {})
            if #savedTiers > 0 then
                tierSys.SelectSpecific(savedTiers)
            end
            if #savedVariants > 0 then
                varSys.SelectSpecific(savedVariants)
            end
        end
    end)
    
    -- Apply Event Teleport settings
    pcall(function()
        selectedEventName = ConfigSystem.Get("Teleport.LastEventSelected", nil)
    end)
    
    -- Apply Webhook settings
    pcall(function()
        currentWebhookURL = ConfigSystem.Get("Webhook.URL", "")
        currentDiscordID = ConfigSystem.Get("Webhook.DiscordID", "")
        webhookTextBox.Text = currentWebhookURL
        discordIDTextBox.Text = currentDiscordID
    ...
    -- Apply Hide Stats settings
    pcall(function()
        currentFakeName = ConfigSystem.Get("Settings.HideStats.FakeName", "Guest")
        currentFakeLevel = ConfigSystem.Get("Settings.HideStats.FakeLevel", "1")
        fakeNameTextBox.Text = currentFakeName
        fakeLevelTextBox.Text = currentFakeLevel
    ...
    -- Apply Freecam settings
    pcall(function()
        local savedSpeed = ConfigSystem.Get("CameraView.Freecam.Speed", 50)
        local savedSens = ConfigSystem.Get("CameraView.Freecam.Sensitivity", 0.3)
        if FreecamModule then
            FreecamModule.SetSpeed(savedSpeed)
            FreecamModule.SetSensitivity(savedSens)
        end
    end)
    
    -- Apply Auto Sell Timer interval
    pcall(function()
        local savedInterval = ConfigSystem.Get("Shop.AutoSellTimer.Interval", 5)
        if AutoSellTimer then
            AutoSellTimer.SetInterval(savedInterval)
        end
    end)
    
    -- Apply Auto Buy Weather settings
    pcall(function()
        local savedWeathers = ConfigSystem.Get("Shop.AutoBuyWeather.SelectedWeathers", {})
        if #savedWeathers > 0 and AutoBuyWeather then
            AutoBuyWeather.SetSelected(savedWeathers)
        end
    end)
    
    -- Apply Skin Animation settings
    pcall(function()
...
-- ✅ MANUAL SAVE BUTTON
makeButton(catConfig, "💾 Save Config", function()
    if ConfigSystem then
        local success, message = ConfigSystem.Save()
        if success then
            SendNotification("Config", "✓ Configuration saved successfully!", 3)
        else
...
-- ✅ MANUAL LOAD BUTTON
makeButton(catConfig, "🔄 Load Config", function()
    if LoadConfigToUI() then
        SendNotification("Config", "✓ Configuration loaded successfully!", 3)
    end
end)
...
makeButton(catConfig, "🔃 Reset to Default", function()
    if ConfigSystem then
        ...
        if success then
            ...
            for _, toggle in pairs(ToggleReferences) do
                if toggle and toggle.setOn then
                    pcall(function() toggle.setOn(false, true) end)
                end
            end
            
            -- Reset text fields
            currentFakeName = "Guest"
            currentFakeLevel = "1"
            currentWebhookURL = ""
            currentDiscordID = ""
            fakeNameTextBox.Text = currentFakeName
...
-- OPENING ANIMATION
-- ============================================
local openThread = task.delay(0, function()
...
ConnectionManager:AddThread(openThread)

-- ============================================
-- ✅ REMOVED: AUTO-APPLY CONFIG ON STARTUP
-- Config is now only loaded when user clicks "Load Config" button
-- ============================================
...
-- MEMORY LEAK PREVENTION: Periodic garbage collection
task.spawn(function()
    while true do
        task.wait(60)
...
-- Cleanup on player leave/teleport
local player = Players.LocalPlayer
ConnectionManager:Add(player.OnTeleport:Connect(function()
    ConnectionManager:Cleanup()
...
SendNotification("✨ Lynx GUI v2.3.1", "Loaded! " .. loadedModules .. "/" .. totalModules .. " modules ready.\n⚠ Manual Save Mode: Use buttons to save/load config", 6)
