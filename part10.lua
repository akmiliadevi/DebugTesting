...
-- ============================================
local function makeMultiSelectDropdown(parent, label, options, callback)
    local dropdownFrame = new("Frame",{
        Parent=parent,
        Size=UDim2.new(1, 0, 0, 40),
        BackgroundColor3=colors.bg4,
...
makeToggle(catSupport, "Real Ping & FPS Panel", function(on)
    if on then
        PingFPSMonitor:Show()
...
makeToggle(catAutoFav, "Enable Auto Favorite", function(on)
    autoFavEnabled = on
...
-- Multi-select dropdown untuk weather
local weatherDropdown = makeMultiSelectDropdown(
    ...
    function(selected)
        ...
        -- Optional: Show notification saat selection berubah
        if #selectedWeathers > 0 then
            local weatherList = table.concat(selectedWeathers, ", ")
            print("🌦️ Weather selected: " .. weatherList)
        end
    end
)
...
-- Discord User ID Input (Optional)
makeTextBox(
    catWebhook,
...
local rarityDropdown = makeMultiSelectDropdown(
    ...
    function(selected)
        ...
        local statusText = #selectedRarities > 0 
            ...
            or "Filter cleared - All rarities"
        print("🎯 " .. statusText)
    end
)
...
linkButton.MouseButton1Click:Connect(function()
    setclipboard("https://discord.gg/lynxx") -- Copy link ke clipboard
